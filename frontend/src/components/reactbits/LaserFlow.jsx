...
export const LaserFlow = ({
    ...
    const currentDprRef = useRef(1); // Keep track of DPR, but don't change it dynamically
    ...
    useEffect(() => {
        ...
        // cleanup previous renderer if exists (shouldn't happen with strict dep array, but safety first)
        if (rendererRef.current) {
            const oldCanvas = rendererRef.current.domElement;
            if (mount.contains(oldCanvas)) mount.removeChild(oldCanvas);
        ...
        // Use fixed DPR or window DPR, capped at 2 for performance
        const targetDpr = Math.min(dpr ?? (window.devicePixelRatio || 1), 2);
        ...
        const uniforms = {
            ...
            uFade: { value: hasFadedRef.current ? 1 : 0 } // continue fade if already faded
        };
        ...
        const setSizeNow = () => {
            if (!mount) return;
            // Fallback to 1 to prevent 0-size WebGL errors but still initialize
        ...
        const animate = () => {
            ...
            // Optional: Skip if document hidden (extra check)
            ...
            // Limit large dt steps (e.g. after tab switch)
            const cdt = Math.min(0.1, dt);

            // Calculate FPS just for debug or potential future use (removed dynamic DPR)
            // const dtMs = dt * 1000;
            ...
            // Handle edge case where lerp might produce NaN if alpha is invalid? (Should actl be safe)
            if (isNaN(mouseSmooth.x)) mouseSmooth.set(0, 0);
        ...
        return () => {
            ...
            // Helpful to explicitly lose context if removing from DOM to prevent "Too many active WebGL contexts"
            // But THREE.Renderer.dispose() basically does the internal cleanup. 
            // We can't easily force context loss without extension, but disposal is best practice.
            renderer.dispose();

            if (mount && mount.contains(canvas)) mount.removeChild(canvas);
            rendererRef.current = null;
        };
    }, [dpr, color]); // Dependencies: Re-init if base DPR preference changes or color changes radically (though color is also uniform)

    // Separate useEffect for uniforms to avoid full re-init
    useEffect(() => {
